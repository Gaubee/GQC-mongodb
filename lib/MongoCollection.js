...
function install(client) {
	...
	collection_methods_with_promise.reduce(function(proto, method_info) {
		var index;
		if (method_info.params && method_info.params.some((param, i) => param.name === "query" && (index = String(i)))) {
			proto[method_info.name] = function() {
				const col = this.col;
				var query = arguments[index];

				JSON_to_query || (JSON_to_query = require("./JSON_to_query"));
